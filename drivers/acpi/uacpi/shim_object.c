...
static uacpi_status acpi_to_uacpi_object(union acpi_object *in,
					 uacpi_object **out);

static uacpi_status acpi_to_uacpi_package(union acpi_object *in,
					  uacpi_object **out)
{
	...
	for (array.count = 0; array.count < in->package.count; array.count++) {
		st = acpi_to_uacpi_object(&in->package.elements[array.count],
					 ...
					 uacpi_object **out)
{
	...
	case ACPI_TYPE_PACKAGE:
		st = acpi_to_uacpi_package(in, out);
		if (st != UACPI_STATUS_OK)
			return st;
		break;
	default:
		acpi_os_printf(
			"ACPI: WARN: acpi_to_uacpi_object does not support type %x\n",
			in->type);
...
static uacpi_status acpi_to_uacpi_object_list(struct acpi_object_list *in,
					      uacpi_object_array *out)
{
	...
	for (out->count = 0; out->count < in->count; out->count++) {
		st = acpi_to_uacpi_object(&in->pointer[out->count],
...
static uacpi_status shim_compute_required_extra_size(uacpi_object *object,
						     uacpi_size *out_size)
{
	uacpi_data_view data_view;
	uacpi_status st;
	...
	case UACPI_OBJECT_STRING:
		st = uacpi_object_get_string(object, &data_view);
		if (st != UACPI_STATUS_OK)
			return st;
	...
	default:
		acpi_os_printf(
			"ACPI: WARN: shim_compute_required_extra_size does not support type %s\n",
...
static uacpi_status shim_write_object(uacpi_object *in, union acpi_object *out,
				      u8 **data_pointer,
				      uacpi_namespace_node *node)
{
	...
	default:
		acpi_os_printf(
			"ACPI: WARN: shim_write_object does not support %s\n",
					...
					uacpi_namespace_node *node)
{
	...
	required_size = 0;
	st = shim_compute_required_extra_size(in, &required_size);
	if (st != UACPI_STATUS_OK)
	...
	st = shim_write_object(in, out->pointer, &extra_data_pointer, node);
	if (st != UACPI_STATUS_OK && out->length == ACPI_ALLOCATE_BUFFER)
				 ...
				 struct acpi_buffer *return_buffer)
{
	...
	st = acpi_to_uacpi_object_list(external_params, &args);
	if (st != UACPI_STATUS_OK)
		return uacpi_convert_status(st);
	...
	if (return_buffer && ret) {
		acpi_status acpi_st =
			uacpi_to_acpi_object(ret, return_buffer, node);
...
acpi_status acpi_get_type(acpi_handle handle, acpi_object_type *ret_type)
{
	...
	st = uacpi_to_acpi_type(object_type, ret_type);
	return uacpi_convert_status(st);
}
...
acpi_status acpi_acquire_mutex(acpi_handle handle, acpi_string pathname,
			       u16 timeout)
{
	WARN(1, "acpi_acquire_mutex is not implemented by uACPI");
	return AE_SUPPORT;
}

acpi_status acpi_release_mutex(acpi_handle handle, acpi_string pathname)
{
	WARN(1, "acpi_release_mutex is not implemented by uACPI");
	return AE_SUPPORT;
}
